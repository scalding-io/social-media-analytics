...
class CMSketch(args: Args) extends Job(args) {
  ...
  // val heavyHittersPct :Double = 0.0001D

  implicit val cmsketchMonoid = new CountMinSketchMonoid(eps, delta, seed) // , heavyHittersPct)
    ...
    .map('sketch -> 'sketch) { s: Option[CMS] =>
      s match {
        case Some(sketch) => {
          ...
          val heavyHitters: Set[Long] = sketch.heavyHitters
          println(" + Heavy Hitters : " + heavyHitters.size)
          heavyHitters.foreach( userid => { println("  - User ID : " + userid + " with estimated cardinality : " + sketch.frequency(userid).estimate) } )
        }
